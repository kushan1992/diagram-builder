...
export default function DashboardPage() {
  ...
  const confirmedDeleteDiagram = async (diagramId: string) => {
    if (!user) return;
    try {
  ...
  const handleDeleteDiagram = async (diagramId: string) => {
    toast(
      <ConfirmToast
        onConfirm={() => confirmedDeleteDiagram(diagramId)}
        onCancel={() => router.push("/dashboard")}
        icon={<Trash2 size={20} className="text-red-600" />}
        title="Delete Diagram"
        subTitle="Are you sure you want to delete this diagram? This action cannot be undone."
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md cursor-pointer"
            >
              Profile
            </button>
          </div>
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 text-sm cursor-pointer"
                    >
                      {userRole === "viewer" ? (
                        <>
                          <Eye size={16} />
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
                Create
              </button>
              <button
                ...
                className="flex-1 px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-900 dark:text-white rounded-md hover:bg-gray-400 dark:hover:bg-gray-500 cursor-pointer"
              >
                Cancel
              </button>
            </div>
